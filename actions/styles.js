...
export const fetchJsonTest = () => {
  return (dispatch, getState) => {
    ...
    return getFromApi(`applications/${checksum}/jsontest.json`).then(response => console.log('jsontest', response))
  }
}
...
export const fetchMessages = () => {
  return (dispatch, getState) => {
    const checksum = getState().admin.currentApp
    return getFromApi(`applications/${checksum}/messages.json`).then(response => dispatch(fetchMessagesFromAws(response.messagesUrl)))
  }
}
...
export const getSignedRequest = file => {
  return (dispatch, getState) => {
    ...
    return makeRequest(
      'GET',
      `/sign-s3?file-name=application-assets/${applicationChecksum}/images/${Date.now()}-${file.name}&file-type=${file.type}`
    )
      .then(response => {
...
export const fetchImages = () => {
  return (dispatch, getState) => {
    const checksum = getState().admin.currentApp
    return getFromApi(`applications/${checksum}/images.json`).then(response => dispatch(fetchImagesFromAws(response.imagesUrl)))
  }
}
...
export const fetchStyles = () => {
  return (dispatch, getState) => {
    const checksum = getState().admin.currentApp
    return getFromApi(`applications/${checksum}/styles.json`).then(response => dispatch(fetchFromAws(response.stylesheetUrl)))
  }
}
