...
const Design = ({
  ...
  <div>
    <Canvas previews={previews} platform={platform} />
    <Sidebar>
      <Header handleClose={handleClose} handleSave={handleSave} busy={saving} />
      <Tabs componentsOrBody={componentsOrBody} showBody={showBody} resetSidebar={resetSidebar} showContentEditor={showContentEditor} />
      <ToolSet>
        {componentsOrBody == 'content' ? (
          <div>
            ...
            {messagesDictionary.map(message => (
              <TextContent key={message.key} keyString={message.key} value={message.value} onChange={handleMessageChange} />
            ))}
          </div>
        ) : null}
        {currentMessageKey && componentsOrBody == 'content' && false ? (
          <TextContent keyString={currentMessageKey} value={currentMessageValue} onChange={handleMessageChange} />
        ) : null}
        {componentsOrBody != 'content' && declarations.length > 0
          ? declarations.map(declaration => (
              <Tool key={declaration.property} property={declaration.property} value={declaration.value} handleChange={handleChange} />
            ))
          : null}
      </ToolSet>
    </Sidebar>
    <BottomBar>
      ...
      <ScreenSelector value={currentScreen} options={screens} handleScreenChange={handleScreenChange} />
