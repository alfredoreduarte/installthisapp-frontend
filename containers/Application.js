	...
	<div>
		...
		{alertTitle ? (
			<Alert bsStyle="warning" onDismiss={handleAlertDismiss} style={{ textAlign: 'center' }}>
				<strong dangerouslySetInnerHTML={{ __html: alertTitle }} /> <span dangerouslySetInnerHTML={{ __html: alertContent }} />
			</Alert>
		) : null}
		<div className="container-flui">{loaded ? children : 'cargando'}</div>
	</div>
)
