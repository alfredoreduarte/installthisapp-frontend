...
const Plans = ({ busy, back, currentPlan, plans, upgrade }) => (
  <div>
    <div className="col-md-12">
      ...
      <h4 className="text-center text-muted" style={{ marginBottom: '50px' }}>
    ...
    <div
      ...
      {plans.map(plan => (
        <div style={{ width: '200px', margin: '0px 32px' }} key={plan.stripeId}>
          <div className={`panel text-center ${plan.id == 'agency_development' ? 'panel-success' : 'panel-default'}`}>
            <div className="panel-body">
              <h5 className="text-uppercase" style={{ marginBottom: '50px', letterSpacing: '1px' }}>
                <b>{plan.name}</b>
              </h5>
              ...
              {currentPlan.stripeId == plan.stripeId ? (
                ...
                <ConditionalBuy
                  ...
                  planId={plan.id}
                  featured={plan.stripeId == 'agency_development'}
                />
              )}
            </div>
          </div>
...
const mapStateToProps = state => {
  ...
  return {
    ...
    currentPlan: state.admin.subscription
      ? _.find(state.plans, { id: state.admin.subscription.planId })
      : { name: 'Free', stripeId: 'free' },
    plans,
  }
}
