...
const Sidebar = ({ installed, checksum, type, children, menu, facebookPageIdentifier, fbAppId, fbAppCanvasId }) => (
	<div className="col-md-2 col-xs-12 ita-main-menu">
		...
		<ul className="list-unstyled">
			...
			{type != 'form' && type != 'fan_gate' && type != 'coupons' && type != 'static_html' && <li><Link to={`/d/apps/${type}/${checksum}/design`} activeClassName="active">Design</Link></li>}
			{(type == 'form' || type == 'fan_gate' || type == 'coupons' || type == 'static_html') && <li><Link to={`/d/apps/${type}/${checksum}/editor`} activeClassName="active">Editor</Link></li>}
		...
		<ul className="list-unstyled">
			...
			<li>
				...
				<ul className="list-unstyled">
					{type != 'form' && type != 'fan_gate' && type != 'static_html' && <li><Link to={`/d/apps/${type}/${checksum}/preferences/specific`} activeClassName="active">App-specific settings</Link></li>}
