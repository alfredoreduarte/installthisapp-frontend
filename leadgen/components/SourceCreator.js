...
const SourceCreator = ({
  ...
  <form className="" onSubmit={e => handleSubmit(e).then(() => reset())}>
    ...
    <div className={`form-group ${selectedFbPageIdentifier || 'hide'}`}>
      ...
      <Field name={'fbFormName'} type="hidden" component="input" />
      {fetchingLeadgenForms ? (
        ...
        <Field
          name="fbFormId"
          component="select"
          className={`form-control ${fbLeadgenForms.length == 0 ? 'hide' : null}`}
          onChange={e => {
          ...
          {fbLeadgenForms.map(leadgenForm => (
            <option key={leadgenForm.id} value={leadgenForm.id} disabled={leadgenForm.status == 'ARCHIVED'}>
      ...
      {selectedFbPageIdentifier && fbLeadgenForms.length == 0 && !fetchingLeadgenForms ? (
        <p>
          ...
          <a href="javascript:void(0);" onClick={() => fetchLeadgenForms(selectedFbPageIdentifier)}>
            Refresh
          </a>
        </p>
      ) : null}
    ...
    {fbLeadgenForms.length == 0 && !fetchingLeadgenForms ? (
      <p>
        Create a{' '}
        <a href={`https://fb.com/${selectedFbPageIdentifier}/publishing_tools/?section=LEAD_ADS_FORMS`} target="_blank" rel="noopener">
          Lead Form
        </a>?
      </p>
    ) : null}
    <div className="form-group">
      <button type="submit" className="btn btn-primary btn-block" disabled={!valid || submitting || fetchingLeadgenForms}>
        Save Source
      </button>
    </div>
  </form>
